...
select city, sum(orders) as Top10Users_Orders 
from (
select user_id,
city,orders, 
row_number() over (partition by city order by orders DESC) as row_num 
from (
...
select count(order_id) as Total_City_Orders, 
city 
from `main-assessment378120.main_assessment.orders`
group by city
) B 
